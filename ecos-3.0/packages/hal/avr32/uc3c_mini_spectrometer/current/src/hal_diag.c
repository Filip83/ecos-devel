...
{
	/*channels[port].usart_dev->brgr = baud_divisor;
	
	channels[port].usart_dev->cr = 
		AVR32_USART_CR_RSTRX_MASK | AVR32_USART_CR_RSTTX_MASK | AVR32_USART_CR_RSTSTA_MASK ;*/
    /*cyg_uint8 _lcr;
...
cyg_hal_plf_serial_init_channel(void* __ch_data)
{
	...
	gpio_enable_module_pin(AVR32_USART2_TXD_1_PIN, AVR32_USART2_TXD_1_FUNCTION);
     ...
     {
	gpio_enable_module_pin(AVR32_USART4_RXD_PIN, AVR32_USART4_RXD_FUNCTION);
	gpio_enable_module_pin(AVR32_USART4_TXD_PIN, AVR32_USART4_TXD_FUNCTION);
	gpio_disable_pin_pull_up(AVR32_USART4_RXD_PIN);
	gpio_disable_pin_pull_up(AVR32_USART4_TXD_PIN);
     }
...
cyg_hal_plf_serial_getc_nonblock(void* __ch_data, cyg_uint8* ch)
{
    ...
    //lsr_global = _lsr;
    CYG_ASSERT((dev->csr & AVR32_USART_CSR_OVRE_MASK) == 0 , "UART receiver overrun error");
