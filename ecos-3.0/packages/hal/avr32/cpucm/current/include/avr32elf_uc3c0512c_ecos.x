...
MEMORY
{
    FLASH (rxai!w) : ORIGIN = 0x80000000, LENGTH = 452k
	USERFLASH (rxai!w) : ORIGIN = 0x80071000, LENGTH = 60K
    CPUSRAM (wxa!ri) : ORIGIN = 0x00000004, LENGTH = 0xFFFC
	HSBSRAM (wxa!ri) : ORIGIN = 0xA0000000, LENGTH = 4K
...
SECTIONS
{
  ...
  .devtab         : {  . = .; KEEP(*( SORT (.ecos.table.*))) ; } >CPUSRAM AT>FLASH
  .data           :
  {
  ...
  _end = .;
  PROVIDE (end = .);
  _heap_start__ = ALIGN(8);
  . = ORIGIN(CPUSRAM) + LENGTH(CPUSRAM) - 1;
  __heap_end__ = .;
  ...
  .stack         ORIGIN(HSBSRAM)/* + LENGTH(CPUSRAM) - _stack_size*/ :
  {
