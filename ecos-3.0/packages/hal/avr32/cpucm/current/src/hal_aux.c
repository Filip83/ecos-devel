...
// Map macros defined in cdl option file to real one
...
{
    int i;
    short *tonull = (0xd0000000);
    // Basic hardware initialization
    hal_bpd_init();
    hal_board_init();
    hal_oscilator_init();
    hal_clocks_init();
#if CYGNUM_HAL_SDRAM_ENABLED  == 1
    // init driver
...
#if defined(CYGNUM_HAL_SMC_NCS0_ENABLED) || \ 
    defined(CYGNUM_HAL_SMC_NCS1_ENABLED) || \
    defined(CYGNUM_HAL_SMC_NCS2_ENABLED) || \
    defined(CYGNUM_HAL_SMC_NCS3_ENABLED) || \
    defined(CYGNUM_HAL_SMC_NCS4_ENABLED) || \
    defined(CYGNUM_HAL_SMC_NCS5_ENABLED)
    smc_init(CYGHWR_HAL_AVR32_CPU_FREQ*1000000);
#endif
...
void hal_oscilator_init(void)
{
    ...
    cyg_uint32 gain = 
    ((CYGNUM_HAL_OSCILATORS_OSC0_FREQV) <  2 ) ? AVR32_SCIF_OSCCTRL0_GAIN_G0 :
    ...
    AVR32_SCIF.oscctrl[0] = 
      (CYGNUM_HAL_OSCILATORS_OSC0_AGC_ENABLE << AVR32_SCIF_OSCCTRL_AGC_OFFSET)
    ...
    | (CYGNUM_HAL_OSCILATORS_OSC0_EXTERNAL_STARTUP_TIME 
            << AVR32_SCIF_OSCCTRL_STARTUP_OFFSET)
    | (CYGNUM_HAL_OSCILATORS_OSC0_ENABLED << AVR32_SCIF_OSCCTRL_OSCEN_OFFSET);
    ...
    cyg_uint32 gain = 
    (CYGNUM_HAL_OSCILATORS_OSC1_FREQV <  900000) ? AVR32_SCIF_OSCCTRL1_GAIN_G0 :
    ...
    AVR32_SCIF.OSCCTRL[1] = 
      (CYGNUM_HAL_OSCILATORS_OSC1_AGC_ENABLE << AVR32_SCIF_OSCCTRL_AGC_OFFSET)
    | (gain << AVR32_SCIF_OSCCTRL_GAIN_OFFSET )
    ...
    AVR32_SCIF.OSCCTRL[1] = 
      (CYGNUM_HAL_OSCILATORS_OSC1_AGC_ENABLE << AVR32_SCIF_OSCCTRL_AGC_OFFSET)
    | (gain << AVR32_SCIF_OSCCTRL_GAIN_OFFSET )
    | (CYGNUM_HAL_OSCILATORS_OSC1_EXTERNAL_STARTUP_TIME 
            << AVR32_SCIF_OSCCTRL_STARTUP_OFFSET)
    | (CYGNUM_HAL_OSCILATORS_OSC1_ENABLED << AVR32_SCIF_OSCCTRL_OSCEN_OFFSET);
    ...
    AVR32_SCIF.oscctrl32 = 
      (AVR32_SCIF_OSCCTRL32_MODE_CRYSTAL_NO_ACG 
	...
	#else
    AVR32_SCIF.oscctrl32 = 
      (AVR32_SCIF_OSCCTRL32_MODE_CRYSTAL_ACG << AVR32_SCIF_OSCCTRL32_MODE_OFFSET)
    | (2 << AVR32_SCIF_OSCCTRL32_STARTUP_OFFSET)
    ...
    AVR32_SCIF.rccr8 = 
            AVR32_SCIF_RCCR8_RCOSC8_EN_MASK |
            ((*calibration_bits)&AVR32_SCIF_RCCR8_CALIB_MASK);
    ...
    AVR32_SCIF.pll[1] = 
    ((PLL_SOURCE(CYGNUM_HAL_OSCILATORS_PLL1_SOURCE)) << AVR32_SCIF_PLLOSC_OFFSET)
...
//-----------------------------------------------------------------------------
// Initalice Brown out detection 5V decabled
void hal_bpd_init()
{
