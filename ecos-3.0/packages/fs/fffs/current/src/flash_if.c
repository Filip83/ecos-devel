...
size_t                len)
{
	...
	GOTO_ON_ERROR(   cyg_dataflash_compare_buf(&priv->dev, 1, page)         );
	RETURN_ON_ERROR( cyg_dataflash_release(&priv->dev)                      );
...
/*
cyg_dataflash_device_t flash_device =
{
	.spi_dev       = &flash_spi_device,
	.polled        = true,
	.blocking      = false
};
*/
//extern struct cyg_flash_dev_funs cyg_dataflash_flash_dev_funs;
...
CYG_DATAFLASH_FLASH_DRIVER(atmel_spi_flash,&flash_spi_device,0x10000000,0,32);
//----------------------------------------------------------------------------
