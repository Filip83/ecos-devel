...
static void (*application_init) (lua_State *L) = NULL;
...
#define lua_readline(L,b,p) \
        ((void)L, fputs(p, L->io_output), fflush(L->io_output),  /* show prompt */ \
        fgets(b, LUA_MAXINPUT, L->io_input) != NULL)  /* get line */
...
static int pushline (lua_State *L, int firstline) {
  ...
  if (l > 0 && b[l-1] == '\n')  /* line ends with newline? */
    b[l-1] = '\0';  /* remove it */
  l = strlen(b);
  if (l > 0 && b[l-1] == '\r')  /* line ends with cr? */
    b[l-1] = '\0';  /* remove it */
  ...
  while ((status = loadline(L)) != -1) {
    ...
    if (status == LUA_OK && lua_gettop(L) > 0) {  /* any result to print? */
      ...
      if (lua_pcall(L, lua_gettop(L)-1, 0, 0) != LUA_OK)
        l_message(L, progname, lua_pushfstring(L,
...
__externC int pmain (lua_State *L) {
  ...
  if (script < 0) {  /* invalid arg? */
    print_usage(L, argv[-script]);
    return 0;
  }
  if (args[has_v]) print_version(L);
...
int lua_main (FILE *output, FILE *input, FILE *error, int argc, char **argv) {
  int status, result;
...
void lua_set_application_init(void(*init)(lua_State*))
{
	application_init = init;
}
