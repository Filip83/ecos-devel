...
//=============================================================================
// Define ltc2704 SPI protocol.
//=============================================================================

//-----------------------------------------------------------------------------
// API function call forward references
static void ltc2704_spi_send_recieve(cyg_spi_device *dev, struct ltc2704_settings *settings, cyg_uint16 data);

static cyg_uint32 ltc2704_tmr_isr(cyg_vector_t vector, cyg_addrword_t data);
static void ltc2704_tmr_dsr(cyg_vector_t vector, cyg_ucount32 count,
                          cyg_addrword_t data);

static void ltc2704_dac_init_clock(void);
static void ltc2704_dac_init_device(cyg_dac_device *device);

static cyg_bool initialized;

__externC cyg_uint32 hal_ltc2704_pclk1;
__externC cyg_uint32 hal_ltc2704_pclk2;

//-----------------------------------------------------------------------------
// Utility function to setup and fetch data via SPI

static void
ltc2704_spi_send_recieve(cyg_spi_device *dev, struct ltc2704_settings *settings, cyg_uint16 data) {
    const cyg_uint8 tx_buf1[3] = { 
        0x06                << 4 |  // Write and update span
        settings->select,           // Address
        0,
        settings->gain      << 1 |  // Gain
        settings->polarity,         // Polarity
        };
    cyg_uint8       rx_buf[3];

    // Carry out SPI transfer.
    cyg_spi_transaction_transfer(dev, true, 3, tx_buf1, rx_buf, true);

    const cyg_uint8 tx_buf2[3] = { 
        0x07                << 4 |  // Write and update data
        settings->select,           // Address
        (data & 0xFF00) >> 8, data & 0xFF };

    // Carry out SPI transfer.
    cyg_spi_transaction_transfer(dev, true, 3, tx_buf2, rx_buf, true);
}
...
ltc2704_dac_init(struct cyg_devtab_entry *tab)
{
    ...
    ltc2704_dac_init_device(device);

    // Set default rate
    ltc2704_dac_set_rate(chan, chan->device->config.rate);
...
Cyg_ErrNo
ltc2704_dac_lookup(struct cyg_devtab_entry **tab,
                 ...
                 const char *name)
{
    ...
    // opened, then it is automatically enabled and output starts to
    // accumulate - so we start the channel now
    chan->enabled = true;
    ltc2704_dac_enable(chan);
...
void
ltc2704_dac_enable(cyg_dac_channel *chan)
{
    ltc2704_dac_info *info = chan->device->dev_priv;
    cyg_uint32 reg;
    cyg_bool start;
...
void
ltc2704_dac_disable(cyg_dac_channel *chan)
{
    ltc2704_dac_info *info = chan->device->dev_priv;
    cyg_uint32 reg;
...
// any steps needed to change the generation rate of the channel, or of the entire
// device. We use a timer channel to generate the interrupts for outputting the
// analog channels.

void
ltc2704_dac_set_rate( cyg_dac_channel *chan, cyg_uint32 rate)
{
    cyg_dac_device *device = chan->device;
    ltc2704_dac_info *info = device->dev_priv;
    cyg_uint32 clock;
...
// in response to a CYG_IO_SET_CONFIG_DAC_GAIN config operation. Only six gains
// are supported. The interpretation of these gains is up to
// to the specific driver.
void
ltc2704_dac_set_gain( cyg_dac_channel *chan, cyg_uint32 gain)
{
    cyg_dac_device *device = chan->device;;
    ltc2704_dac_info *info = (ltc2704_dac_info *) device->dev_priv;
    ltc2704_dac_setup *setup = (ltc2704_dac_setup *) info->setup;
    struct ltc2704_settings *settings = &setup->channel_settings[chan->channel];

    switch (gain) {
    case DAC_GAIN1 : settings->gain = LTC2704_GAIN1; break;
    case DAC_GAIN2 : settings->gain = LTC2704_GAIN2; break;
    case DAC_GAIN3 : settings->gain = LTC2704_GAIN3; break;
    default: break;
    }
}
...
// in response to a CYG_IO_SET_CONFIG_DAC_POLARITY config operation. Two
// polarities are supported: unipolar and bipolar.
void
ltc2704_dac_set_polarity( cyg_dac_channel *chan, cyg_uint32 polarity)
{
    cyg_dac_device *device = chan->device;;
    ltc2704_dac_info *info = (ltc2704_dac_info *) device->dev_priv;
    ltc2704_dac_setup *setup = (ltc2704_dac_setup *) info->setup;
    ...
    settings->polarity = polarity == DAC_POL_BIPOLAR ? LTC2704_POL_BI : LTC2704_POL_UNI;
...
static cyg_uint32
ltc2704_tmr_isr(cyg_vector_t vector, cyg_addrword_t data)
{
...
static void
ltc2704_tmr_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data)
{
    cyg_dac_device *device = (cyg_dac_device *) data;
    ltc2704_dac_info *info = (ltc2704_dac_info *) device->dev_priv;
    ltc2704_dac_setup *setup = (ltc2704_dac_setup *) info->setup;
    ...
    while (chan_active) {
        if (chan_active & 0x1) {
            ...
            if (!(res & CYG_ISR_CALL_DSR)) {
                ...
                CYGHWR_HAL_KINETIS_GPIO_CLEAR_PIN(E, 5);
                ltc2704_spi_send_recieve(info->spi_dev, &setup->channel_settings[(*chan)->channel], output);
...
#else
    sim_p->scgc6 |= CYGHWR_HAL_KINETIS_SIM_SCGC6_FTM(CYGNUM_DEVS_DAC_LTC2704_DAC0_TMR);
#endif
#endif
}
...
static void
ltc2704_dac_init_device(cyg_dac_device *device)
{
    ltc2704_dac_info *info = device->dev_priv;
    cyg_uint32 reg;
