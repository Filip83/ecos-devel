...
//      msg_queue.c
//
//      Message Queeue implementation to pass interrupt events to app.
//
//==========================================================================
...
#include <pkgconf/devices_msg_queue.h>
...
DS_GET(cyg_uint8* regs)
{
    ...
    if (1 != cyg_i2c_transaction_tx(&cyg_i2c_wallclock_pcf2129a, true, tx_data, 1, false)) {
        // The device has not responded to the address byte.
        ok = false;
    } else {
        // Now fetch the data
        cyg_i2c_transaction_rx(&cyg_i2c_wallclock_pcf2129a, true, regs, 9, true, true);
...
Cyg_WallClock::init_hw_seconds(void)
{
  ...
  AVR32_AST.clock = ( AVR32_AST_CLOCK_CSSEL_32_KHZ_CLOCK << AVR32_AST_CLOCK_CSSEL_OFFSET);
  ...
  AVR32_AST.cr = (14 << AVR32_AST_CR_PSEL_OFFSET) | AVR32_AST_CR_CAL_MASK |
                 AVR32_AST_CR_EN_MASK | AVR32_AST_CR_PCLR_MASK;

  while((AVR32_AST.sr & AVR32_AST_SR_BUSY_MASK))
  {
  }

  //initialize time to zero e.g. Unix Epoch
  init_pcf_hwclock();
