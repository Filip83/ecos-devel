...
//      m25qxx.c
//
//      SPI flash driver for Micron N25Qxx devices
//      and compatibles.
//
//=============================================================================
...
// Date:        2013-04-04
// Purpose:     Micron N25Qxx SPI flash driver
//              implementation
//
//####DESCRIPTIONEND####
//
...
#include <pkgconf/devs_flash_spi_n25qxx.h>
...
{                                                       \
    ...
    do {                                                \
        HAL_DELAY_US (15);                              \
        dev_status = n25qxx_spi_rdsr ( dev );           \
...
typedef struct n25qxx_params {
    cyg_uint16 sector_count[3];       // Number of sectors on device.
    cyg_uint32 sector_size[3];        // Supported sector size(s) in Byte
    cyg_uint32 jedec_id;              // 3 byte JEDEC identifier for this device.
} n25qxx_params;


static const n25qxx_params n25qxx_supported_devices [] = {
    // Some Micron parts.
    // Note1: Not all parts can support sector size of 4 or 64 KBytes
    { // Support for SST 1 GBit devices (N25Q00AA13G).
        sector_count        : {32768, 2048, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba21
    },
    { // Support for SST 512 MBit devices (N25Q512A).
        sector_count        : {16384, 1024, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba20
    },
    { // Support for SST 256 MBit devices (N25Q256A).
        sector_count        : {8192, 512, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba19
    },
    { // Support for SST 128 MBit devices (N25Q128A).
        sector_count        : {4096, 256, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba18
    },
    { // Support for SST 64 MBit devices (N25Q064A).
        sector_count        : {2048, 128, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba17
    },
    { // Support for SST 32 MBit devices (N25Q032A).
        sector_count        : {1024, 64, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba16
    },
    { // Support for SST 16 MBit devices (N25Q016A).
        sector_count        : {512, 32, 0},
        sector_size         : {SZ_4K, SZ_64K, 0},
        jedec_id            : 0x0020ba15
    },
    //
...
static cyg_bool
n25qxx_to_local_addr(struct cyg_flash_dev *dev, cyg_flashaddr_t * addr)
{
...
n25qxx_spi_rdid(struct cyg_flash_dev *dev)
{
...
static inline void
n25qxx_spi_wren(struct cyg_flash_dev *dev)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
...
static  void
n25qxx_spi_wrdis(struct cyg_flash_dev *dev)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
...
static  void
n25qxx_spi_se(struct cyg_flash_dev *dev, cyg_flashaddr_t addr)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
    cyg_uint8       tx_buf[4] = 
    { 
...
static cyg_uint8
n25qxx_spi_rdsr(struct cyg_flash_dev *dev)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
...
static  void
n25qxx_spi_read(struct cyg_flash_dev *dev, cyg_flashaddr_t addr,
...
static int
n25qxx_init(struct cyg_flash_dev *dev)
{
    n25qxx_params *dev_params = (n25qxx_params *) n25qxx_supported_devices;
    cyg_uint32      device_id;
    cyg_uint8       i = 0;
    int             retval = FLASH_ERR_INVALID;

    // Find the device in the supported devices list.
    device_id = n25qxx_spi_rdid(dev);
...
static int
n25qxx_erase_block(struct cyg_flash_dev *dev, cyg_flashaddr_t block_base)
{
    ...
    if (n25qxx_to_local_addr(dev, &local_base)) 
    {
        n25qxx_spi_wren(dev);
        n25qxx_spi_se(dev, local_base);
...
static int n25qxx_verifi(struct cyg_flash_dev *dev, cyg_flashaddr_t local_base,
                         const void *data, size_t len)
{
    ...
    while(len)
    {
        ...
        n25qxx_spi_read(dev, local_base, verifi_buf, rx_bytes);
...
static int
n25qxx_program(struct cyg_flash_dev *dev, cyg_flashaddr_t base,
                const void *data, size_t len)
{
    ...
    {
        n25qxx_spi_wren(dev);
        n25qxx_spi_pp(dev, local_base, tx_ptr, tx_bytes);
    ...
    retval = n25qxx_verifi(dev,vlocal_base ,data, len);
out:
    return retval;
}
...
static int
n25qxx_read(struct cyg_flash_dev *dev, const cyg_flashaddr_t base,
             void *data, size_t len)
{
    ...
    if (n25qxx_to_local_addr(dev, &local_base)) 
    {
        while (rx_bytes_left) 
        {
            ...
            n25qxx_spi_read(dev, local_base, rx_ptr, rx_bytes);
...
static int
n25qxx_lock(struct cyg_flash_dev *dev, cyg_flashaddr_t base)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
    cyg_flashaddr_t local_base = base;
    int             retval = FLASH_ERR_INVALID;
...
static int
n25qxx_unlock(struct cyg_flash_dev *dev, cyg_flashaddr_t base)
{
    cyg_spi_device *spi_device = (cyg_spi_device *) dev->priv;
    cyg_flashaddr_t local_base = base;
    int             retval = FLASH_ERR_INVALID;
...
CYG_FLASH_FUNS (
    cyg_devs_flash_spi_n25qxx_funs, // Exported name of function pointers.
    n25qxx_init,                    // Flash initialization.
    cyg_flash_devfn_query_nop,       // Query operations not supported.
    n25qxx_erase_block,             // Sector erase.
    n25qxx_program,                 // Program multiple pages.
    n25qxx_read,                    // Read arbitrary amount of data.
    n25qxx_lock,                    // Locking (lock the whole device).
    n25qxx_unlock
);
