...
// Enabling FTM timing check may interfere with operation of the ADC. To use
// this remove J15 jumper and put a scope on pin 2.
#define ADC1859_TIMING_DEBUG 0

// Enabling causes driver to mask interrupts in the timing device instead
// of eCos.
#define ADC1859_HW_INT_MASK 1

//-----------------------------------------------------------------------------
// Diagnostic support
// Switch the #if to 1 to generate some diagnostic messages.

#ifdef CYGPKG_DEVS_ADC_CORTEXM_LTC1859_TRACE
...
//=============================================================================
// Define ltc1859 SPI protocol.
//=============================================================================

//-----------------------------------------------------------------------------
// API function call forward references
static cyg_int16 ltc1859_spi_send_recieve(cyg_spi_device *dev, struct ltc1859_settings *settings);

static cyg_uint32 ltc1859_tmr_isr(cyg_vector_t vector, cyg_addrword_t data);
static void ltc1859_tmr_dsr(cyg_vector_t vector, cyg_ucount32 count,
                          cyg_addrword_t data);

static void ltc1859_adc_init_clock(void);
static void ltc1859_adc_init_device(cyg_adc_device *device);

static cyg_bool initialized;

__externC cyg_uint32 hal_ltc1859_pclk1;
__externC cyg_uint32 hal_ltc1859_pclk2;

//-----------------------------------------------------------------------------
// Utility function to setup and fetch data via SPI

static cyg_int16
ltc1859_spi_send_recieve(cyg_spi_device *dev, struct ltc1859_settings *settings)
{
    const cyg_uint8 tx_buf[2] = { 
        settings->mode      << 7 | 
        settings->opposite  << 6 | 
        settings->select    << 4 | 
        settings->polarity  << 3 | 
        settings->gain      << 2 | 
        settings->activity, 0x00 };
    cyg_uint8       rx_buf[2];
    cyg_int16       retval = 0;

    // Carry out SPI transfer.
    cyg_spi_transaction_transfer(dev, true, 2, tx_buf, rx_buf, true);

    // Make return code
    retval |= ((cyg_uint32)rx_buf[0]) << 8;
    retval |= ((cyg_uint32)rx_buf[1]);
...
ltc1859_adc_init(struct cyg_devtab_entry *tab)
{
    ...
    ltc1859_adc_init_device(device);

    // Set default rate
    ltc1859_adc_set_rate(chan, chan->device->config.rate);
...
Cyg_ErrNo
ltc1859_adc_lookup(struct cyg_devtab_entry **tab,
...
void
ltc1859_adc_enable(cyg_adc_channel *chan)
{
    ltc1859_adc_info *info = chan->device->dev_priv;
    cyg_uint32 reg;
    cyg_bool start;
    cyg_uint32 mask;
    cyg_uint8 mask_pos;
    ...
    mask = info->chan_mask;
    active_pos = 0;
    for (mask_pos = 0; mask_pos < 8; mask_pos++)
        if ((mask & (1 << mask_pos)))
...
void
ltc1859_adc_disable(cyg_adc_channel *chan)
{
    ltc1859_adc_info *info = chan->device->dev_priv;
    cyg_uint32 reg;
    cyg_uint32 mask;
...
void
ltc1859_adc_set_rate( cyg_adc_channel *chan, cyg_uint32 rate)
{
    cyg_adc_device *device = chan->device;
    ltc1859_adc_info *info = device->dev_priv;
    cyg_uint32 clock;
...
// in response to a CYG_IO_SET_CONFIG_ADC_GAIN config operation. Only two gains
// are supported: high and low. The interpretation of these gains is up to
// to the specific driver.
void
ltc1859_adc_set_gain( cyg_adc_channel *chan, cyg_uint32 gain)
{
    cyg_adc_device *device = chan->device;;
    ltc1859_adc_info *info = (ltc1859_adc_info *) device->dev_priv;
    ltc1859_adc_setup *setup = (ltc1859_adc_setup *) info->setup;
    struct ltc1859_settings *settings = &setup->channel_settings[chan->channel];

    settings->gain = gain == ADC_GAIN_LOW ? LTC1859_GAIN_LOW : LTC1859_GAIN_HI;
}
...
void
ltc1859_adc_set_polarity( cyg_adc_channel *chan, cyg_uint32 polarity)
{
    cyg_adc_device *device = chan->device;;
    ltc1859_adc_info *info = (ltc1859_adc_info *) device->dev_priv;
    ltc1859_adc_setup *setup = (ltc1859_adc_setup *) info->setup;
    ...
    settings->polarity = polarity == ADC_POL_BIPOLAR ? LTC1859_POL_BI : LTC1859_POL_UNI;
}

//-----------------------------------------------------------------------------
// This function is called from the generic ADC package to set the ADC mode
// in response to a CYG_IO_SET_CONFIG_ADC_MODE config operation. Two modes 
// are supported, single ended and differential.
void
ltc1859_adc_set_mode( cyg_adc_channel *chan, cyg_uint32 mode)
{
    cyg_adc_device *device = chan->device;;
    ltc1859_adc_info *info = (ltc1859_adc_info *) device->dev_priv;
    ltc1859_adc_setup *setup = (ltc1859_adc_setup *) info->setup;
    ...
    settings->mode = mode == ADC_MODE_DIFF ? LTC1859_MODE_DIFF : LTC1859_MODE_SGL;
}
...
static cyg_uint32
ltc1859_tmr_isr(cyg_vector_t vector, cyg_addrword_t data)
{
...
static void
ltc1859_tmr_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data)
{
    cyg_adc_device *device = (cyg_adc_device *) data;
    ltc1859_adc_info *info = (ltc1859_adc_info *) device->dev_priv;
    ltc1859_adc_setup *setup = (ltc1859_adc_setup *) info->setup;
    ...
    if (current_chan != 0) {
        ...
        CYGHWR_HAL_KINETIS_GPIO_SET_PIN(E, 5);
        sample = ltc1859_spi_send_recieve(info->spi_dev, &setup->channel_settings[current_chan->channel]);
        ...
        chan_pos++;
        if (chan_pos >= 8 || info->chan_active[chan_pos] == 0)
            chan_pos = 0;
    }
...
#else
    sim_p->scgc6 |= CYGHWR_HAL_KINETIS_SIM_SCGC6_FTM(CYGNUM_DEVS_ADC_LTC1859_ADC0_TMR);
#endif
#endif
}
...
static void
ltc1859_adc_init_device(cyg_adc_device *device)
{
    ltc1859_adc_info *info = device->dev_priv;
    cyg_uint32 reg;
