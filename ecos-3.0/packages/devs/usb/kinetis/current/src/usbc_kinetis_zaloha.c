...
usb_device_khci_state_struct_t kinetis_usb_device =
{
...
static void USB_DeviceKhciInterruptTokenDone(usb_device_khci_state_struct_t *khciState)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if ((!khciState->endpointState[index].transferLength) || (!remainingLength) ||
                (khciState->endpointState[index].stateUnion.stateBitField.maxPacketSize > length))
            {
                ep0.buffer_size = khciState->endpointState[index].transferDone;
                if (isSetup)
                {
                    //ep0.control_buffer = bdtBuffer;
                ...
                {
                    ep0.buffer  = khciState->endpointState[index].transferBuffer;
                }
                khciState->endpointState[index].stateUnion.stateBitField.transferring = 0U;
            }
            else
           ...
           /* CYG_ASSERT((usbs_control_return (*)(usbs_control_endpoint*, int))0 != ep0.common.complete_fn, \
            ...
            ep0.complete_fn      = (usbs_control_return (*)(usbs_control_endpoint*, int)) 0;*/
             ...
             cyg_uint32 pos = 0;
                ...
                ep0.buffer_size = pos;
                USB_DeviceKhciSend(&kinetis_usb_device,0,
                        ep0.buffer, ep0.buffer_size);
        }
        else
        {
    ...
    else
    {
        ...
        /*CYG_ASSERT (UC3C_USB_ENDPOINTS > endpoint && endpoint, "Invalid end point");
...
{
    usb_device_endpoint_init_struct_t initStr;
    initStr.endpointAddress = 0;
    initStr.maxPacketSize   = 8;
    initStr.transferType    = 0;
    initStr.zlt             = 0;

    USB_DeviceKhciControl(&kinetis_usb_device,
            kUSB_DeviceControlEndpointInit,&initStr);
    
    initStr.endpointAddress = 0x80;
    USB_DeviceKhciControl(&kinetis_usb_device,
            kUSB_DeviceControlEndpointInit,&initStr);
    
    USB_DeviceKhciControl(&kinetis_usb_device,
        kUSB_DeviceControlRun,&initStr);
}
...
void usbs_kinetis_init (void)
{
    ...
    // Reste USB
    khciState->registerBase->USBTRC0 = 0x80;
    while(khciState->registerBase->USBTRC0 & 0x80)
    {
...
usbs_kinetis_control_setup()
{
    ...
    if ((protocol != USB_DEVREQ_TYPE_STANDARD) || !handled) 
    {
        ...
        status = usbs_parse_host_get_command (&ep0);
   ...
   /* else
    {
        USB_DeviceKhciPrimeNextSetup(&kinetis_usb_device);
    }*/
