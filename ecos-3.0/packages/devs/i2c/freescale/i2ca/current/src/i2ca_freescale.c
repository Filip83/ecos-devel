...
// Contributors:  Tomas Frydrych <tomas@sleepfive.com>
...
#include <cyg/io/i2ca_freescale.h>
...
//#include <cyg/hal/registers/regsi2c.h> // From SDK

#ifdef CYGPKG_DEVS_I2C_FREESCALE_I2CA_TRACE
...
CYG_MACRO_START                           \
    __i2c->i2cr &= ~FREESCALE_I2C_I2CR_MSTA_M; \
    __i2c->i2cr &= ~FREESCALE_I2C_I2CR_MTX_M;  \
CYG_MACRO_END

#define I2C_START(__i2c)                 \
CYG_MACRO_START                          \
    __i2c->i2cr |= (FREESCALE_I2C_I2CR_MTX_M | FREESCALE_I2C_I2CR_MSTA_M); \
CYG_MACRO_END

#define I2C_REPEAT_START(__i2c)           \
CYG_MACRO_START                           \
    __i2c->i2cr |= FREESCALE_I2C_I2CR_RSTA_M; \
CYG_MACRO_END
...
static const dev_i2c_freescale_frequency_entry_t dev_i2c_freescale_freq_table1[]
                 = { CYGHWR_IO_FREESCALE_I2C_FREQUENCY_TABLE1 };
static const dev_i2c_freescale_frequency_entry_t dev_i2c_freescale_freq_table2[]
                 = { CYGHWR_IO_FREESCALE_I2C_FREQUENCY_TABLE2 };
static const cyg_uint32 dev_i2c_freescale_freq_table_size1 =
                 sizeof(dev_i2c_freescale_freq_table1) /
                 sizeof(dev_i2c_freescale_freq_table1[0]);
static const cyg_uint32 dev_i2c_freescale_freq_table_size2 =
                 sizeof(dev_i2c_freescale_freq_table2) /
                 sizeof(dev_i2c_freescale_freq_table2[0]);
...
static cyg_uint8
freescale_i2c_freq_div_calc1(cyg_uint32 period_sp)
{
    cyg_uint32 sys_clk_per;
    cyg_uint32 period, the_period = 0xffffffff, agr;
    cyg_uint32 mult, the_mult = 1;
    cyg_int32 icr_i, the_icr_i = dev_i2c_freescale_freq_table_size1 - 1;
    cyg_uint16 icr;
    cyg_uint8 i2cf = 0xff;
    ...
    agr = FREESCALE_I2C_DELAY_IS_AGRESIVE(period_sp);
	...
	for(icr_i = dev_i2c_freescale_freq_table_size1 - 1; icr_i >= 0; icr_i--) {
		icr = dev_i2c_freescale_freq_table1[icr_i];
		period = sys_clk_per * (icr * mult);

#if I2CA_DEBUG_CLOCK >= 3
		...
		if (period == period_sp) {
			...
			I2C_TRACE ("FreqDiv EQ: [0x%02x] the_mult = %d, icr[0x%02x] = %d\n",
					   i2cf, the_mult, the_icr_i, dev_i2c_freescale_freq_table1[the_icr_i]);
			return FREESCALE_I2C_F(icr_i);
			...
			if(0 == fit) {
				...
				i2cf = FREESCALE_I2C_F(the_icr_i);

#ifdef CYGPKG_DEVS_I2C_FREESCALE_I2CA_TRACE
                    I2C_TRACE ("FreqDiv LT: [0x%02x] the_mult = %d, icr[0x%02x] = %d\n",
                               i2cf, the_mult, the_icr_i, dev_i2c_freescale_freq_table1[the_icr_i]);
                    icr = dev_i2c_freescale_freq_table1[the_icr_i];
...
static cyg_uint8
freescale_i2c_freq_div_calc2(cyg_uint32 period_sp)
{
    cyg_uint32 sys_clk_per;
    cyg_uint32 period, the_period = 0xffffffff, agr;
    cyg_uint32 mult, the_mult = 1;
	...
	for(icr_i = dev_i2c_freescale_freq_table_size1 - 1; icr_i >= 0; icr_i--) {
		icr = dev_i2c_freescale_freq_table2[icr_i];
		period = sys_clk_per * (icr * mult);
		...
		if (period == period_sp) {
			...
			I2C_TRACE ("FreqDiv EQ: [0x%02x] the_mult = %d, icr[0x%02x] = %d\n",
					   i2cf, the_mult, the_icr_i, dev_i2c_freescale_freq_table2[the_icr_i]);
			return FREESCALE_I2C_F(icr_i);
		} else if (period < period_sp) {
			if(0 == fit) {
				if(agr && ((period_sp - period) < period - the_period )) {
				...
				i2cf = FREESCALE_I2C_F(the_icr_i);
                    ...
                    I2C_TRACE ("FreqDiv LT: [0x%02x] the_mult = %d, icr[0x%02x] = %d\n",
                               i2cf, the_mult, the_icr_i, dev_i2c_freescale_freq_table2[the_icr_i]);
                    icr = dev_i2c_freescale_freq_table2[the_icr_i];
                    period = (sys_clk_per * (icr * the_mult));
                    I2C_TRACE ("    Actual: period = %d.%d[ns], frequency = %d[Hz]\n",
                               period/I2C_CLOCK_NORM, period%I2C_CLOCK_NORM,
                               (cyg_uint32)((1000000000 * I2C_CLOCK_NORMLL) / period));
...
{
    cyg_uint32 period;
    cyg_uint32 period_actual1, period_actual2;
    cyg_uint32 delta1, delta2;
    cyg_uint8 freq, freq1, freq2;
    cyg_uint32 sys_clk_per;
    ...
    if((period = *period_p) & FREESCALE_I2C_DELAY_F_IS_CACHED_M) {
        ...
        freq1 = freescale_i2c_freq_div_calc1(period);
        freq2 = freescale_i2c_freq_div_calc2(period);
		period_actual1 = sys_clk_per * dev_i2c_freescale_freq_table1[freq1];
		period_actual2 = sys_clk_per * dev_i2c_freescale_freq_table2[freq2];
		delta1 = period_actual1 > period * I2C_CLOCK_NORM ? period_actual1 - (period * I2C_CLOCK_NORM) : (period * I2C_CLOCK_NORM) - period_actual1;
		delta2 = period_actual2 > period * I2C_CLOCK_NORM ? period_actual2 - (period * I2C_CLOCK_NORM) : (period * I2C_CLOCK_NORM) - period_actual2;
		if (delta1 < delta2)
			freq = freq1;
		else
			freq = 0x20 | freq2;
...
wait_bus_ready(freescale_i2c_extra* extra)
{
    ...
    if (!extra->i2c_owner) {
        ...
        if((
            ...
            __sr =
#endif
            i2c_s->i2sr) & FREESCALE_I2C_I2SR_IBB_M)
        {
            bus_ready = false;
            ...
            {
                if(!(i2c_s->i2sr & FREESCALE_I2C_I2SR_IBB_M)) {
                    bus_ready = true;
                    break;
                }
            }
...
freescale_i2c_isr(cyg_vector_t vec, cyg_addrword_t data)
{
    ...
    cyg_drv_interrupt_mask(extra->i2c_isr_id);
    cyg_drv_interrupt_acknowledge(extra->i2c_isr_id);
    ...
    if (FREESCALE_I2C_XFER_MODE_TX == extra->i2c_mode) {
         ...
         */
        if (i2c_s->i2cr & FREESCALE_I2C_I2CR_MTX_M) {
            /*
            ...
            if (sr & FREESCALE_I2C_I2SR_IAL_M) {
                ...
                I2C_STOP(i2c_s);
                I2C_CLEAR(i2c_s, FREESCALE_I2C_I2SR_IBB_M);
                ...
                i2c_s->i2cr &= ~FREESCALE_I2C_I2CR_MTX_M;

                // ACK if more bytes
                if (extra->i2c_count != 1)
                    i2c_s->i2cr &= ~FREESCALE_I2C_I2CR_TXAK_M;
                // NACK if asked to
                ...
                dummy_read = i2c_s->i2dr;
            }
        }
        /*
         * Already in RX mode ...
         */
        else if (sr & FREESCALE_I2C_I2SR_IAL_M) {
...
//                HAL_INTERRUPT_ACKNOWLEDGE (extra->i2c_isr_id);
            ...
            // extract data
            *(extra->i2c_data.i2c_rx_data) = i2c_s->i2dr;
...
freescale_i2c_handle_xfer(cyg_i2c_device * dev, int address)
{
    ...
    if (extra->send_start) {
        I2C_TRACE ("Doing start: restart: %s\n",
                   i2c_s->i2cr & FREESCALE_I2C_I2CR_MSTA_M ? "yes" : "no");

        /*
         * If we are the current master, use repeat start
         */
        if (i2c_s->i2cr & FREESCALE_I2C_I2CR_MSTA_M) {
        ...
        // Wait until busy
        sr = i2c_s->i2sr;
        while (!(sr & FREESCALE_I2C_I2SR_IBB_M)) {
            sr = i2c_s->i2sr;
        }

        I2C_TRACE ("STATUS: sr 0x%02x, cr 0x%02x\n", i2c_s->i2sr, i2c_s->i2cr);

        // Clear IAL and IIF bits
        i2c_s->i2sr = 0;
        ...
        if (sr & FREESCALE_I2C_I2SR_RXAK_M)
        	extra->i2c_count++;
    } else {
...
freescale_i2c_init(struct cyg_i2c_bus *bus)
{
    ...
    cyg_drv_interrupt_set_cpu(extra->i2c_isr_id, 0);
#endif
...
//#endif
    ...
    i2c_s->i2cr |= FREESCALE_I2C_I2CR_IEN_M;
    i2c_s->i2cr |= FREESCALE_I2C_I2CR_IIEN_M;
...
#include <cyg/io/i2ca_freescale_buses.inl>

//---------------------------------------------------------------------------
// EOF i2ca_freescale.c
